...
/**
 * MkadminSessionResource
 */
@RequestScoped
@Path("/auth")
@Produces(MediaType.APPLICATION_JSON)
public class MkadminSessionResource {
	...
	public Response logout(@CookieParam(
		value = MkadminServerApp.CLIENT_COOKIE_PREFIX + CommonHttpUtils.NAME_SESSIONID_COOKIE) final String sessionId) {
		...
		return Response.ok(ResponsePayload.messageOnly(MessagePayload.info("Sie haben sich erfolreich ausgeloggt")))
			.cookie(CommonHttpUtils.createSessionInvalidatedCookie(MkadminServerApp.CLIENT_COOKIE_PREFIX)).build();
