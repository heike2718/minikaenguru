 ...
 */
@RequestScoped
public class KatalogeRepository {

	private static final String UNBEKANNT = "Unbekannt";

	private static final Logger LOGGER = LoggerFactory.getLogger(KatalogeRepository.class);
	...
	public List<Schule> findSchulen(final String searchTerm) {
		...
		TypedQuery<Schule> query = em.createNamedQuery(Schule.FIND_SCHULEN_MIT_NAME, Schule.class);
		query.setParameter("name", "%" + searchTerm.toLowerCase() + "%").setParameter("excluded", UNBEKANNT);
	...
	public List<Ort> findOrte(final String searchTerm) {
		...
		LOGGER.debug("searchTearm={}, Anzahl Orte={}", searchTerm, resultList.size());
		return resultList;
	}
	...
	public Optional<Land> findLandWithKuerzel(final String landKuerzel) {
		...
		try {
			...
			LOGGER.debug("nicht gefunden: {} - {}", "Land", landKuerzel);
	...
	public Optional<Ort> findOrtWithKuerzel(final String ortKuerzel) {
		...
		try {
			...
			LOGGER.debug("nicht gefunden: {} - {}", "Ort", ortKuerzel);
			return Optional.empty();
		} catch (NonUniqueResultException e) {
	...
	public int countSchulenMitKuerzel(final String kuerzel) {
		...
		TypedQuery<Long> query = em.createNamedQuery(Schule.COUNT_WITH_KUERZEL, Long.class).setParameter("kuerzel", kuerzel);
